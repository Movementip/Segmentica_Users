...
import { FiChevronDown, FiExternalLink, FiPrinter, FiSave, FiX } from 'react-icons/fi';
import { DocumentPreviewZoomControls } from '../DocumentPreviewControls/DocumentPreviewControls';
...
export const RecordDocumentCenter = ({ documents, buttonClassName, saveTarget, onSaved }: RecordDocumentCenterProps): JSX.Element | null => {
    ...
    const openExternalAndClosePreview = (url: string) => {
        openInNewTabWithUnlock(url, {
            onBeforeOpen: closePreview,
        });
    };
    ...
    const openDocument = async () => {
        if (pdfOpenUrl) {
            openExternalAndClosePreview(pdfOpenUrl);
            return;
        }
        ...
        try {
            ...
            openExternalAndClosePreview(openUrl);
        } catch (error) {
            ...
            if (fallbackUrl) {
                openExternalAndClosePreview(fallbackUrl);
            }
        } finally {
            setPdfLoading(false);
        }
    ...
    const printDocument = async () => {
        ...
        if (!frame) {
            openInNewTabWithUnlock(objectUrl, {
                onBeforeOpen: closePreview,
            });
            return;
        }
