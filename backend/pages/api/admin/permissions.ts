...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const cols = await getTableColumns('permissions');
        const idCol = pickCol(cols, ['id']);
        const keyCol = pickCol(cols, ['key', 'perm_key', 'permission_key']);
        const nameCol = pickCol(cols, ['name', 'title']);
        const descCol = pickCol(cols, ['description', 'desc']);

        if (!idCol || !keyCol) {
            return res.status(500).json({ error: 'RBAC: таблица permissions не содержит обязательные колонки' });
        }
        ...
        if (req.method === 'POST') {
            ...
            const r = await query(
                `INSERT INTO public.permissions(${colsToInsert.join(', ')})
                 VALUES (${valuesSql})
        ...
        if (req.method === 'DELETE') {
            ...
            await query(`DELETE FROM public.permissions WHERE ${idCol} = $1`, [id]);
