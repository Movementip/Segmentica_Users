  ...
  if (req.method === 'GET') {
    try {
      ...
      const result = await query(`
        SELECT 
          п.*,
          COUNT(ап.id) as количество_товаров,
          COALESCE(SUM(з."общая_сумма"), 0) as общая_сумма_закупок
        FROM "Поставщики" п
        LEFT JOIN "Ассортимент_поставщиков" ап ON п.id = ап."поставщик_id"
        LEFT JOIN "Закупки" з ON п.id = з."поставщик_id"
        GROUP BY п.id, п."название", п."телефон", п."email", п."рейтинг", п."created_at"
