...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'GET') {
        const actor = await requirePermission(req, res, 'transport.list');
        if (!actor) return;
        try {
      ...
      `)
        ...
        const actor = await requirePermission(req, res, 'transport.create');
        if (!actor) return;
        try {
      ...
      `, [название, телефон || null, email || null, тариф || null]);
        ...
        const actor = await requirePermission(req, res, 'transport.delete');
        if (!actor) return;
        try {
            const { id } = req.query;
