...
export default function LoginPage(): JSX.Element {
    ...
    const canSubmit = useMemo(() => {
        return Boolean(selectedEmployee?.id) && password.trim().length > 0 && !submitting;
    }, [password, selectedEmployee?.id, submitting]);

    const submit = async () => {
        if (!selectedEmployee?.id) {
            setError('Выбери сотрудника из списка');
            return;
        }
        ...
        try {
            ...
            const res = await fetch('/api/auth/login', {
                ...
                body: JSON.stringify({ employee_id: selectedEmployee.id, password, rememberMe }),
            });
                                    ...
                                    <Box className={styles.field}>
                                        <div className={styles.dropdownAnchor}>
                                            <TextField.Root
                                                ...
                                                onBlur={() => {
                                                    window.setTimeout(() => setIsEmployeeFocused(false), 120);
                                                }}
                                                onKeyDown={(e) => {
                                                    if (e.key === 'Enter') {
                                                        e.preventDefault();
                                                        if (suggestions.length === 1) {
                                                            setSelectedEmployee(suggestions[0]);
                                                            setQuery(suggestions[0].fio);
                                                            setSuggestions([]);
                                                        }
                                                    }
                                                }}
